...
def my_imfilter(image, kernel):
    ...
    padding1 = ((k - 1) // 2)
    padding2 = ((l - 1) // 2)
    pad_this = (padding1, padding1), (padding2, padding2), (0,0)
    if len(kernel[0]) % 2 == 0 or len(kernel) % 2 == 0:
        raise Exception("You can not have even dimensions")

    img_padded = np.pad(image, pad_this, 'reflect') 
    for i in range(n):
        for j in range(m):
            for x in range (c):
                filtered_image[j, i, x] = (kernel * img_padded[j : j + k, i : i + l, x]).sum()
...
def gen_hybrid_image(image1, image2, cutoff_frequency):
    ...
    high_frequencies = np.clip(high_frequencies + 0.5, 0, 1)
    ...
    hybrid_image = np.clip(hybrid_image - 0.5, 0, 1)
